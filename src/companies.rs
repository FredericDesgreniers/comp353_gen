...
impl Company {
	pub fn from_text(text: &str) -> Result<Self, String> {
		...
		let first_name = parts.next().ok_or(String::from("No first name"))?.to_string();
		let last_name = parts.next().ok_or(String::from("No last name"))?.to_string();

		let city = parts.next().ok_or(String::from("No city"))?.to_string();
		let province = parts.next().ok_or(String::from("No province"))?.to_string();
		let postal_code = parts.next().ok_or(String::from("No postal code"))?.to_string();
...
pub fn read_companies() -> Result<Vec<Company>, Error> {
	...
	let companies = company_reader.lines().filter_map(|line| {
		if let Ok(line) = line {
