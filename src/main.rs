...
const EMPLOYEE_TYPE: [&str; 6] = ["Development", "QA", "UI", "Design", "BusinessIntelligence", "Networking"];
...
fn main() -> Result<(), Error> {
	...
	writeln!(out, "CREATE TABLE Companies(
	...
	writeln!(out, "CREATE TABLE Contracts(
	...
	for (index, company) in companies.into_iter().enumerate() {
		...
		let email = format!("{}_{}@{}.com", company.first_name, company.last_name, company.name);
		...
		let number_of_contracts = if is_nike{1} else {contract_rng.sample(&mut rng)};
